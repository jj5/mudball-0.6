...
create table t_journal_std_migration (
  ...
  a_std_migration_created_in int unsigned not null default ( @a_std_interaction_rid ),
...
-- for each row
-- begin
--   set new.a_std_migration_updated_in = @a_std_interaction_rid;
-- end;

create trigger bu_t_journal_std_migration
before update on t_journal_std_migration
for each row
begin
  signal sqlstate '45000' set message_text = 'updates are not allowed.';
end;

create trigger bd_t_journal_std_migration
before delete on t_journal_std_migration
for each row
begin
  signal sqlstate '45000' set message_text = 'deletes are not allowed.';
end;

create table t_particle_std_software_code (
...
create trigger bu_t_particle_std_software_code
before update on t_particle_std_software_code
for each row
begin
  signal sqlstate '45000' set message_text = 'updates are not allowed.';
end;
